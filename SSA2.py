...
class SSAv3(object):
    ...
    @staticmethod
    def getcontributions(X=None, s=None, plot=True):
        ...
        if plot:
            ...
            ax.set_title('Компоненты Lambda_i')
            vals = ax.get_yticks()
    ...
    def view_time_series(self):
        ...
        self.ts.plot(title='Оригинальный временной ряд')
        
    def embed(self, embedding_dimension=None, frequency=None, verbose=False, return_df=False):
        ...
        if frequency:
            ...
            self.embedding_dimension = (self.embedding_dimension//self.frequency)*self.frequency
    ...
    def SVD(self, verbose=False):
        ...
        self.s_contributions = self.getcontributions(X, self.s, False)
    ...
    def view_s_contributions(self, adjust_scale=False, cumulative=False, return_df=False):
        ...
        if cumulative:
            contribs['Вклад'] = contribs.Contribution.cumsum()
        if adjust_scale:
            contribs = (1/contribs).max()*1.1-(1/contribs)
        ax = contribs.plot.bar(legend=False)
        ax.set_xlabel("Singulari")
        ax.set_title('Компоненты Singulari {}'.\
    ...
    @classmethod
    def view_reconstruction(cls, *hankel, names=None, return_df=False, plot=True, symmetric_plots=False): #plot=True
        hankel_mat = None
        for han in hankel:
        ...
        title = 'Восстановленый сигнал'
        if names or names==0: 
            title += ' с компонентами{}: {}'
            title = title.format('' if len(str(names))==1 else '', names)
        if plot:
        ...
        if singular_values:
            try:
                ...
                if singular_values==0:
                    ...
                    raise('Список индексов сингулярного значения, используемых для прогноза')
        else:
    ...
    def forecast_recurrent(self, steps_ahead=30, singular_values=None, plot=False, return_df=False, **plotargs):#plot=False
        try:
            self.X_com_hat
        ...
        forecast_df = df(self.ts_forecast, columns=['Прогноз'], index=new_index)
        forecast_df['Оригинал'] = np.append(self.ts_v, [np.nan]*steps_ahead)
        if plot:
            forecast_df.plot(title='Прогнозируемый и исходный временной ряд', **plotargs)
        if return_df:
            return forecast_df
